...
async function main() {
    ...
    const { error: venueErr } = await supabase.from('venues').insert({
        ...
        city_id: '398a39cc-367b-4604-b6b2-1042fde1a041',
    ...
    const { error: secErr } = await supabase.from('venue_sections').insert([
        { id: section1, venue_id: venueId, name: 'General Admission', description: 'Entry to brunch. Food not included.', capacity: 100, color: '#F2A900', sort_order: 1 },
        { id: section2, venue_id: venueId, name: 'VIP Booth', description: 'Includes a bottle of champagne and seating for 4.', capacity: 10, color: '#FF006E', sort_order: 2 },
        { id: section3, venue_id: venueId, name: 'Bottomless Mimosas Add-On', description: 'Add-on only. Must purchase GA or VIP.', capacity: 50, color: '#3B82F6', sort_order: 3 }
    ]);
