    ...
    try {
      ...
      if (mode === "partial") {
        ...
        if (cents > total) {
          setResult({
            type: "error",
            message: "Amount exceeds order total",
          });
          setLoading(false);
          return;
        }
    ...
    <div className="rounded-xl bg-white/[0.03] border border-border-subtle p-4 space-y-3">
      ...
      <div className="flex gap-3">
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <span className="text-sm">
            Full Refund ({`$${(total / 100).toFixed(2)}`})
          </span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full bg-white/5 border border-border-subtle rounded-xl px-4 py-3 text-sm text-white placeholder:text-txt-secondary/50 focus:outline-none focus:border-gold/50 transition-colors resize-none"
        />
      </div>
