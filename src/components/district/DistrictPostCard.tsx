...
interface DistrictPostData {
  ...
  created_at: string;
  author: DistrictPostAuthor | null;
}

interface DistrictPostCardProps {
  post: DistrictPostData;
  district: number;
...
const POST_TYPE_BADGE: Record<string, { label: string; className: string }> = {
  update: { label: "Update", className: "" }, // will use districtColor dynamically
  alert: { label: "Alert", className: "bg-coral/15 text-coral" },
  photo: { label: "Photo", className: "bg-gold/15 text-gold" },
};
...
export default function DistrictPostCard({
  ...
  const canModerate = isCouncilMember || isMyPost;
  const isLong = post.body.length > 280;
  const roleBadge = author?.role ? ROLE_BADGE_MAP[author.role] : null;
  const postTypeCfg = POST_TYPE_BADGE[post.post_type] || POST_TYPE_BADGE.update;
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}/district/${district}`;
    if (navigator.share) {
      try { await navigator.share({ title: "District Post", url }); } catch {}
    } else {
      ...
      <div className="p-4 pb-0">
        ...
        <div className="mb-2">
          {post.post_type === "alert" ? (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-coral/15 text-coral">
            ...
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-gold/15 text-gold">
              {postTypeCfg.label}
            </span>
          ) : (
            <span
              className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold"
              style={{ backgroundColor: `${districtColor}20`, color: districtColor }}
            >
              {postTypeCfg.label}
            </span>
          )}
        ...
        {/* Title for alerts */}
        {post.post_type === "alert" && post.title && (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {roleBadge && roleBadge.label !== "Official" && (
              ...
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-8 z-20 bg-deep border border-border-subtle rounded-xl shadow-xl py-1 min-w-[140px] animate-in fade-in zoom-in-95 duration-150 origin-top-right">
                    ...
                    {isCouncilMember && <div className="border-t border-white/[0.04] my-0.5" />}
                    <button
