...
interface DistrictCommentsSheetProps {
  district: number;
  postId: string;
  isOpen: boolean;
  onClose: () => void;
...
export default function DistrictCommentsSheet({
  district, postId, isOpen, onClose, userId, isCouncilMember, commentCount, onCountChange,
}: DistrictCommentsSheetProps) {
  ...
  const fetchComments = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/districts/${district}/posts/${postId}/comments`);
      if (res.ok) {
  ...
  const handleDelete = async (commentId: string) => {
    try {
      const res = await fetch(`/api/districts/${district}/posts/${postId}/comments/${commentId}`, { method: "DELETE" });
      if (res.ok) {
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-4">
          {loading ? (
            ...
            <div className="text-center py-10">
              ...
              {userId && <p className="text-[10px] text-txt-secondary mt-1">Be the first to comment</p>}
            </div>
          ) : (
            comments.map((comment) => {
              const canDelete = isCouncilMember || comment.author?.id === userId;
              ...
              return (
                <div key={comment.id} className="flex gap-2.5 group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Actions row -- no like button for district comments */}
